...
pub fn criterion_timed_cache_v1(c: &mut Criterion) {
    ...
    group.bench_function(
        ...
        |b| b.iter(|| timed_cache_sequential(black_box((100, 10, 30)))),
    );

    group.bench_function(
        "TimedCache - v1: n_keys: high, value_size: small, max_capacity: high",
        |b| b.iter(|| timed_cache_sequential(black_box((10_000, 10, 1_000)))),
    );

    group.bench_function(
        "TimedCache - v1: n_keys: high, value_size: high, max_capacity: high",
        |b| b.iter(|| timed_cache_sequential(black_box((10_000, 1_000, 5_000)))),
    );
...
pub fn criterion_timed_cache_v2(c: &mut Criterion) {
    ...
    group.bench_function(
        ...
        |b| b.iter(|| timed_cache_v2_sequential(black_box((100, 10, 30)))),
    );

    group.bench_function(
        "TimedCache - v2: n_keys: high, value_size: small, max_capacity: high",
        |b| b.iter(|| timed_cache_v2_sequential(black_box((10_000, 10, 1_000)))),
    );

    group.bench_function(
        "TimedCache - v2: n_keys: high, value_size: high, max_capacity: high",
        |b| b.iter(|| timed_cache_v2_sequential(black_box((10_000, 1_000, 5_000)))),
    );
